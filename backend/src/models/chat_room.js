...
async function schemaFuncs() {
  ...
  await pool.query(createSql, (createErr, createRes) => {
    ...
    if (createRes) {
      ...
      let createTableSql = `CREATE TABLE ${SCHEMA_NAME}.${TABLE_NAMES.chatRoom}(
        id TEXT primary key,
        buyer_id TEXT,
        seller_id TEXT,
        FOREIGN KEY(buyer_id) REFERENCES ${SCHEMA_NAME}.${TABLE_NAMES.users}(id),
        FOREIGN KEY(seller_id) REFERENCES ${SCHEMA_NAME}.${TABLE_NAMES.users}(id)
      );`;
