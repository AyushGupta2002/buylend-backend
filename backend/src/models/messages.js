...
async function schemaFuncs() {
  ...
  await pool.query(createSql, (createErr, createRes) => {
    ...
    if (createRes) {
      ...
      let createTableSql = `CREATE TABLE ${SCHEMA_NAME}.${TABLE_NAMES.messages}(
        id TEXT primary key,
        chat_id TEXT,
        sender_id TEXT,
        message TEXT,
        FOREIGN KEY(chat_id) REFERENCES ${SCHEMA_NAME}.${TABLE_NAMES.chatRoom}(id),
        FOREIGN KEY(sender_id) REFERENCES ${SCHEMA_NAME}.${TABLE_NAMES.users}(id)
      );`;
